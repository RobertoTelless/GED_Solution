...
{
    public class TipoDocumentoService : ServiceBase<TIPO_DOCUMENTO>, ITipoDocumentoService
    {
        private readonly ITipoDocumentoRepository _baseRepository;
        ...
        {
            TIPO_DOCUMENTO item = _baseRepository.GetItemById(id);
            return item;
        }

        public TIPO_DOCUMENTO CheckExist(TIPO_DOCUMENTO item, Int32? idAss)
        {
            TIPO_DOCUMENTO volta = _baseRepository.CheckExist(item, idAss);
            return volta;
        }

        public List<TIPO_DOCUMENTO> GetAllItens(Int32? id)
        {
        ...
        public Int32 Create(TIPO_DOCUMENTO item, LOG log)
        {
        ...
        public Int32 Create(TIPO_DOCUMENTO item)
        {
            using (DbContextTransaction transaction = Db.Database.BeginTransaction(IsolationLevel.ReadCommitted))
            {
                try
        ...
        public Int32 Edit(TIPO_DOCUMENTO item)
        {
            using (DbContextTransaction transaction = Db.Database.BeginTransaction(IsolationLevel.ReadCommitted))
            {
                try
        ...
        public Int32 Delete(TIPO_DOCUMENTO item, LOG log)
        {
            using (DbContextTransaction transaction = Db.Database.BeginTransaction(IsolationLevel.ReadCommitted))
            {
                try
